...
// Platform: explicit --platform flag, or auto-detect from OS
const platformIdx = args.indexOf('--platform')
const platformMap = { win32: 'win', linux: 'linux', darwin: 'mac' }
const platform    = platformIdx >= 0 && args[platformIdx + 1]
                    ? args[platformIdx + 1]
                    : (platformMap[process.platform] ?? 'win')
...
if (platform === 'linux' && process.platform === 'win32') {
  warn('Cross-compiling Linux from Windows — AppImage requires symlink privileges.')
  warn('If this fails, use GitHub Actions CI or build on a Linux machine.')
}
...
print(`${R}${B}    ╚═══╝  ╚═╝  ╚═╝╚══════╝ ╚═════╝ ╚═╝  ╚═╝${RST}`)
print('')
print(`${W}${B}  Desktop Client — ${platformLabel} Build Pipeline${RST}`)
print(`${DIM}  ─────────────────────────────────────────────────────${RST}`)
...
// Linux:   bump → compile → package [→ release]
const TOTAL = (isWin ? 4 : 3) + (release ? 1 : 0)
let stepNum = 0

// ── Step: Version bump ───────────────────────────────────────────────────────
step(++stepNum, TOTAL, 'Bumping version…')
if (bumpType !== 'none') {
...
// ── Step: Installer assets (Windows only) ────────────────────────────────────
if (isWin) {
  step(++stepNum, TOTAL, 'Generating installer assets…')
  run('node scripts/generate-assets.mjs')
}

// ── Step: Compile ────────────────────────────────────────────────────────────
step(++stepNum, TOTAL, 'Compiling Electron app…')
