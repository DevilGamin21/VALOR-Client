...
export default function Discover() {
  ...
  return (
    <div className="pb-8 pt-6">
      <h1 className="px-6 font-bold text-white mb-4 text-lg">Discover</h1>
    ...
    return (
      <div className="px-6">
        <button
          ...
          className="flex items-center gap-1 text-white/50 hover:text-white mb-4 transition-colors text-sm"
        >
          <ChevronLeft size={16} /> Back to Collections
        </button>
        <h2 className="text-white font-bold mb-4 text-base">{expanded.name}</h2>
        {detailLoading ? (
          <Spinner />
        ) : detail ? (
          <>
            {detail.overview && (
              <p className={`text-white/50 mb-4 max-w-2xl ${'text-sm'}`}>{detail.overview}</p>
            )}
            ...
            {detail.items.length === 0 && (
              <p className={`text-white/40 ${'text-sm'}`}>No items in this collection.</p>
            )}
          </>
        ) : (
        ...
        <div className={'grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4'}>
          {collections.map((col) => (
            <button
              ...
              className="group relative rounded-lg overflow-hidden text-left outline-none bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 transition-all"
            >
              {col.posterUrl ? (
              ...
              <div className="p-2">
                <p className={`text-white font-medium truncate ${'text-xs'}`}>{col.name}</p>
                <p className={`text-white/40 ${'text-[10px]'}`}>{col.itemCount} item{col.itemCount !== 1 ? 's' : ''}</p>
              </div>
            </button>
          ))}
        </div>
...
function StreamingTab({ onPlay }: { onPlay: (item: UnifiedMedia) => void }) {
  ...
  return (
    <div className={'px-6'}>
      ...
      {provider == null && (
        <p className={`text-white/40 ${'text-sm'}`}>Select a streaming service to browse.</p>
      )}
          ...
          {totalPages > 1 && results.length > 0 && (
            <div className="flex items-center justify-center gap-4 mt-2">
              <button
                ...
                disabled={page <= 1}
                className="p-2 rounded-lg bg-white/10 text-white/60 hover:bg-white/15 disabled:opacity-30 disabled:cursor-not-allowed"
              >
                <ChevronLeft size={16} />
              </button>
...
function RecommendationsTab() {
  return (
    <div className="px-6">
      <div className="rounded-xl border border-white/10 bg-white/5 text-center max-w-md mx-auto p-8">
        <p className="text-white/70 font-medium mb-1 text-sm">Recommendations</p>
        <p className="text-white/40 text-xs">Coming soon — personalised picks based on your watch history.</p>
      </div>
    </div>
  )
}
